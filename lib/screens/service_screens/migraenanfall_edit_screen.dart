...
class MigraeneanfallEditScreen extends StatelessWidget {
  int id;
  late MAService service;
  late Migraeneanfall anfall;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("Edit Migraeneanfall"),
      ),
      body: Container(
        child: Column(
        ...
        child: Column(
          children: <Widget>[
            ...
            _buildFormTextField(
              name: 'schmerzen',
              labelText: 'Schmerzen',
              value: anfall.schmerzen,
            ),
            const SizedBox(height: 15),
            _buildFormTextField(
              name: 'lokalisation',
              labelText: 'Lokalisation',
              value: anfall.lokalisation,
            ),
          ],
        ),
      ),
...
// do something sensible here
  ...
  Future<bool> _loadUser() async {
    try {
      anfall = await service.getMigraeneanfallById(id: this.id);
      return true;
    } on ObjectNotFoundException catch (e) {
      anfall = Migraeneanfall.empty();
      return false;
    }
    return false;
  }
